...
--Stored procedure for getting processing Range information
...
-- Process using dynamic sql for more flexibility  | This procedure takes 1 capture instance parameter that is parted in f and t lsn
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM cdc.welcome WHERE capture_instance = @capture_instance)
	BEGIN
		RAISEERROR('The specified capture instance is not valid for the current cdc configuration', 16, 1);
		RETURN;
	END
	...
	SELECT @fromlsn = ISNULL(last_processed_lsn, sys.fn_cdc_get_min_lsn(@capture_instance)),
...
EXECUTE dbo.GetCDCProcessingRange @capture_instance = N'welcome', @from_lsn = @from_lsn OUTPUT, @to_lsn = @to_lsn OUTPUT;

SELECT @from_lsn AS from_lsn, @to_lsn AS to_lsn;

-- Do processing of records -- TFV usage explained in next slide and demo
...
WHERE parameter = 'column parameter'
GO
...
SELECT @fromlsn AS fromlsn, @to_lsn AS to_lsn;
...
UPDATE dbo.cdc_lsn_tracking
SET last_processed_lsn = @to_lsn
WHERE capture_instance = N'dbo.welcome';
GO