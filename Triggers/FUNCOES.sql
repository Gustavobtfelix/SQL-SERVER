--FUNCOES DEVEM ESTAR SEMPRE NO COMEÇO DO ALGORITMO

--SELECT DE REFERENCIA
SELECT SUM(QUANTIDADE * [PREÇO]) FROM [ITENS NOTAS FISCAIS]
WHERE NUMERO = 100
--FUNCAO QUE REALIZA O SELECT DE ACORDO COM O PARAMETRO
...
--FUNCAO QUE CONTA NOTAS FISCAIS NA DATA ESPECIFICADA COMO PARAMETRO
...
BEGIN
   ...
   IF @CONTADOR_NOTAS > 0 
   --UTILIZA A FUNCAO FATURAMENTO
       BEGIN
          INSERT INTO @TABELA_NUMEROS ([NUMERO], [STATUS], [FATURAMENTO]) VALUES (@LIMITE_MINIMO, 'e nota fiscal', [dbo].[FaturamentoNota](@LIMITE_MINIMO))
	   END
   ELSE
       BEGIN
          INSERT INTO @TABELA_NUMEROS ([NUMERO], [STATUS], [FATURAMENTO]) VALUES (@LIMITE_MINIMO, 'Nao e nota fiscal', 0)
	   END
...
--FUNCAO QUE RETORNA TABELAS @TEMPORARIAS
...
--FUNCAO QUE RETORNA O NUMERO DE NOTAS EM UM PERIODO DE TEMPO
...
SELECT * FROM [dbo].[FuncTabelaNotas]('20170101','20170110')
--DELETA FUNCAO
DROP FUNCTION [dbo].[FuncTabelaNotas]
